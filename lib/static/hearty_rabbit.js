...
{
	let cover_intent = Intent.make("view", coll.owner, coll.coll, coll.cover, new URLSearchParams("rendition=thumbnail"));
...
function add_element(blob_id)
{
	...
	assign_image_to_entry(entry, get_blob_intent(blob_id, "thumbnail").url(), element.filename);
...
function main()
{
	...
	document.querySelector("#move-blob-dialog form").addEventListener("submit", ev =>
	{
		...
		{
			queue_action({
				move: get_blob_intent(document.getElementById("move-blob-dialog").dataset.blob_id),
...
{
	let url = intent.coll != null ? intent.view().url() :
