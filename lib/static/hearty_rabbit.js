...
function add_blob(blob_id)
{
	...
	img.src = "/blob/" + dir.username + "/" + blob_id;
...
function upload_file(file)
{
	...
	req.open("PUT", "/upload/" + dir.username + "/" + dir.collection + "/" + file.name);
	req.onload = () =>
	{
...
{
	show_lightbox("/blob/" + dir.username + "/" + blob_id);
...
function delete_image()
{
	if (confirm("Do you want to delete this image"))
	{
		...
		let req = new XMLHttpRequest();
		req.open("GET", "/unlink/" + dir.username + "/" + blob_id);
		req.onload = () =>
		{
			// extract blob_id from the location
			if (req.status === 202)
