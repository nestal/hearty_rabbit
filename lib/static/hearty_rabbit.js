...
function add_coll(coll)
{
	...
	entry.querySelector("img").src = path_url_with_collection("blob", coll, dir.colls[coll].cover) + "?thumbnail";;
...
function add_blob(blob_id)
{
	...
	entry.querySelector("img").src = path_url("blob", blob_id) + "?thumbnail";
...
function main()
{
	...
	history.replaceState(dir, "", document.location.pathname);
	list_dir();
}
...
function list_dir()
{
	...
	else
	{
		...
		console.log("checking displaying login dialog", dir.login_message);
