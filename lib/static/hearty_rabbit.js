...
function show_image(blob_id, push_state)
{
	...
	// show metadata
	show_image_meta({filename:blob_id});
...
function show_image_meta(intent_fields)
{
	return new Promise((resolve, reject) =>
	{
		...
		req.onload = () =>
		{
			...
			if (req.status === 200)
			{
				console.log("meta = ", req.response);
				document.getElementById("meta-filename").textContent = req.response.mime;
				document.getElementById("meta-datetime").textContent = req.response.phash;
				resolve();
			}
			else
			{
