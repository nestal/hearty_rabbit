...
function main()
{
	...
	window.onpopstate = ev =>
	{
		...
		update_dir(Intent.current()).then(()=>{list_dir();});
	};
	...
	for (let i = 0; i < close_btns.length; i++)
		close_btns[i].addEventListener(
			"click", ()=>
			{
				...
				history.pushState(null, intent.coll, intent.url());
...
{
	return update_dir(intent).then(location =>
	{
		// update URL bar
...
function update_dir(intent)
{
	// Issue AJAX to update the parent collection of the intent.
	// That is, if the intent points to a blob, update the collection that contains that blob.
	let parent_dir = intent.parent_collection();

	let location = (parent_dir == null) ? "/" : parent_dir.view().url();
	let ajax_url = (parent_dir != null && parent_dir.coll != null) ? parent_dir.api().url() :
	...
	return new Promise((resolve, reject)=>
	{
		...
		req.onload = () =>
		{
			if (req.status === 200)
			{
				...
				// Show image if the intent arguments points to a blob.
...
{
	let intent = get_blob_intent(blob_id);
	console.log("intent", intent.query.toString());
	if (push_state)
		history.pushState(null, blob_id, intent.view().url());
	...
	console.log("before api url", intent, intent.api().url());
	console.log("intent", intent.query.toString());

	// show metadata
	get_image_meta(intent.api()).then(meta =>
	{
	...
	console.log("after api url", intent, intent.api().url());
	console.log("intent", intent.query.toString());

	show_lightbox(intent.api().url());
