...
function get_blob_url(blobid, rendition)
{
	...
	{
		let url = "/api/" + owner + "/" + coll + "/" + blobid;
		if (rendition != null || dir.auth != null)
		{
			url += "?";
			if (rendition != null)
			{
...
{
	show_toast("uploading " + file.name + " file " +
		upload_queue.length > 0 ? "(" + upload_queue.length + " pending)" : ""
	);
