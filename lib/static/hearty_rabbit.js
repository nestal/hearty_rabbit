...
function add_coll(coll)
{
	...
	entry.querySelector("div.text-overlay-box span").textContent = (coll.length > 0 ? coll : default_album);
...
function add_blob(blob_id)
{
	...
	entry.querySelector("div.text-overlay-box span").textContent = element.filename;
...
function main()
{
	...
	document.getElementById("close-lightbox").addEventListener("click", hide_image, false);
...
function update_permission_icons(blob_id)
{
	...
	const perm_icon = entry.querySelector("div.perm-overlay-box i.material-icons");
