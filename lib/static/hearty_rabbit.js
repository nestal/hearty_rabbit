...
function get_blob_url(blobid, rendition)
{
	...
	if (coll != null && owner != null)
		return "/api/" + owner + "/" + coll + "/" + blobid + (rendition != null ? "?" + rendition : "");
	else
...
function add_coll(coll)
{
	...
	assign_image_to_entry(
		entry,
		path_url_with_collection("api", coll, dir.colls[coll].cover) + "?thumbnail",
...
function queue_upload_file(file)
{
	console.log("queuing file " + file.name);

	upload_queue.push(file);
	if (current_upload == null)
		upload_file(upload_queue.shift());
}

function upload_file(file)
{
	show_toast("uploading " + file.name + " file (" + file.type + ")");

	current_upload = new XMLHttpRequest();
	current_upload.open("PUT", path_url("upload", file.name));
	current_upload.onload = () =>
	{
		// extract blob_id from the location
		if (current_upload.status === 201)
		{
			const blob_id = current_upload.getResponseHeader("location").substr(-40);
		...
		console.log("upload complete. starting next download");
		if (upload_queue.length > 0)
			upload_file(upload_queue.shift());
		else
			show_toast(null);
	};
	current_upload.onerror = () =>
	{
		alert("onerror(): cannot upload file: " + req.statusText);
		close_overlays(false);
	};
	current_upload.send(file);
}
...
function main()
{
	...
	document.getElementById("file-upload").addEventListener("change", function()
	{
		for (let i = 0; i < this.files.length; i++)
			queue_upload_file(this.files[i]);
	});
	...
	document.getElementById("create-album-form").addEventListener("submit", ev =>
	{
		ev.preventDefault();
	...
	document.getElementById("move-blob-form").addEventListener("submit", ev =>
	{
		ev.preventDefault();
...
function change_dir(collection, onsuccess)
{
	...
	req.open("GET", collection != null ?
		path_url_with_collection("api", collection, "", "json") :
		"/query/collection?user=" + dir.username + "&json"
	);
...
function paste(e)
{
	for (let i = 0 ; i < e.clipboardData.items.length ; i++)
	{
		...
		if (item.type.indexOf("image") !== -1)
			queue_upload_file(item.getAsFile());
    }
}
