...
function show_image(blob_id, push_state)
{
	if (push_state)
	{
		let intent = get_blob_intent(blob_id).view();
		history.pushState(null, blob_id, intent.url());
	}
	...
	let intent = get_blob_intent(blob_id).api();

	show_lightbox(intent.url());
