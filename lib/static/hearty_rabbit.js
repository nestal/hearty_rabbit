...
function add_blob(blob_id)
{
	...
	assign_image_to_entry(entry, path_url("view", blob_id) + "?thumbnail", element.filename);
...
{
	let req = new XMLHttpRequest();
	req.open("GET", path_url_with_collection("list", collection, ""));
	req.onload = () =>
	{
		// extract blob_id from the location
		if (req.status === 200)
...
function show_image(blob_id, push_state)
{
	...
	show_lightbox(path_url("view", blob_id));
