...
{
	return "/" + action + "/" + dir.username + (dir.collection.length > 0 ? "/" : "") + dir.collection + "/" + filename;
}
...
{
	console.log("uploading", file.username, file.type, "file");
...
function main()
{
	...
	if (window.dir && dir.username.length > 0)
	{
		...
		document.getElementById("username").textContent = dir.username;
		document.getElementById("go-home-nav").style.display = "inline";
	...
	{
		document.getElementById("go-home-nav").style.display = "none";
