...
{
	return path_url_with_collection(action, dir.collection ? dir.collection : "", filename);
}
...
function add_coll(coll)
{
	...
	assign_image_to_entry(
		entry,
		path_url_with_collection("blob", coll, dir.colls[coll].cover) + "?thumbnail",
...
function assign_image_to_entry(entry, image_url, description)
{
	...
	img.addEventListener("error", ev => {img.src = "/lib/rabbit_head.svg";});
	img.src = image_url;
	img.alt = description;
...
function add_blob(blob_id)
{
	...
	assign_image_to_entry(entry, path_url("blob", blob_id) + "?thumbnail", element.filename);
...
function main()
{
	...
	document.getElementById("move-blob-btn").addEventListener("click", ev =>
	{
		post_blob(
			document.getElementById("move-blob-dialog").dataset.blob_id,
			"move="+document.getElementById("dest-album-name").value,
...
function change_dir(collection)
{
	let req = new XMLHttpRequest();
	req.open("GET", path_url_with_collection("coll", collection, ""));
	req.onload = () =>
	{
		// extract blob_id from the location
		if (req.status === 200)
...
{
	if (push_state)
		history.pushState(window.dir, "", path_url("view", ""));
...
{
	let req = new XMLHttpRequest();
	req.open("GET", "/listcolls/" + dir.username + "/");
	req.onload = () =>
	{
		// extract blob_id from the location
		if (req.status === 200)
