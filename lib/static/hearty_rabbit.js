...
function list_dir()
{
	...
	if (dir.elements)
	{
		...
		// Load images
		let sorted_blobs = Object.keys(dir.elements);
		sorted_blobs.sort((a,b) =>
		{
			return dir.elements[a].timestamp - dir.elements[b].timestamp;
		});
		let index = 0;
		sorted_blobs.forEach(blob_id =>
		{
			index++;
			dir.elements[blob_id].filename = index.toString() + " -> " + dir.elements[blob_id].timestamp;
			add_element(blob_id);
		});
	}
	else if (dir.colls)
