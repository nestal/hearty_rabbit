...
function add_coll(coll)
{
	...
	const text = entry.querySelector("div.text-overlay-box span");
	text.textContent = (coll.length > 0 ? coll : "(default album)");
...
function add_blob(blob_id)
{
	...
	const text = entry.querySelector("div.text-overlay-box span");
	text.textContent = element.filename;
...
function main()
{
	...
	{
		ev.preventDefault();
		document.getElementById("create-album-dialog").style.display = "block";
	}, false);
	...
	if (dir.elements)
	{
		document.getElementById("headline").innerHTML = "Album: " + dir.collection;
	...
	{
		document.getElementById("headline").innerHTML = "List of " + dir.username + "&#39;s Albums";
	...
	if (window.dir && dir.username.length > 0)
	{
		...
		document.getElementById("username-nav").style.display = "inline-block";
