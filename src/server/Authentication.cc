...
Salt random_salt()
{
	...
	return random_value<Salt>();
}

const int min_iteration = 5000;
const std::string& default_hash_algorithm()
{
	static const std::string hash{"sha512"};
	return hash;
};
...
void Authentication::create_session(
...
)
{
	...
	auto session = random_value<SessionID>(system_random);

	db.command(
		[
			session, username=std::string{username}, this, completion = std::move(completion)
		](auto&&, std::error_code ec) mutable
		{
			// TODO: handle errors from "reply"
			m_session = session;
			m_uid = UserID{username};
			completion(ec);
		},
...
{
	auto new_cookie = random_value<SessionID>(system_random);
...
std::optional<SessionID> Authentication::parse_cookie(const Cookie& cookie)
{
