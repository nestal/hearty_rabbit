...
)
{
	...
	Authentication auth{secure_random<UserID::SessionID>(), username};
	db.command(
		[
			auth,
		...
		auth.id().session().data(), auth.id().session().size(),
...
std::string Authentication::set_cookie(std::chrono::seconds session_length) const
{
	...
	if (valid())
	{
		boost::algorithm::hex_lower(m_uid.session().begin(), m_uid.session().end(), std::back_inserter(result));
	...
	db.command(
		[comp=std::move(completion), *this, new_cookie](auto&& reply, auto ec)
		{
			...
			else
			{
				...
				comp(ec, Authentication{new_cookie, id().user()});
			}
		},
