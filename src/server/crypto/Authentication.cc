...
)
{
	...
	Authentication auth{secure_random<Authentication::CookieID>(), username};
	db.command(
		[
			auth,
...
Authentication::Authentication(CookieID cookie, std::string_view user, bool guest) :
...
std::optional<Authentication::CookieID> parse_cookie(std::string_view cookie_str)
{
	Cookie cookie{cookie_str};

	auto value = cookie.field("id");
	if (Authentication::CookieID result{}; value.size() == result.size() * 2)
	{
...
{
	auto new_cookie = secure_random<Authentication::CookieID>();
