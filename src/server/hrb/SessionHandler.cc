...
void SessionHandler::on_upload(UploadRequest&& req, EmptyResponseSender&& send)
{
	...
	if (m_auth.id().user() != path_url.user())
	{
...
/*		pdb.exact_match(phash=*blob.phash(), [blob=blob.ID()](auto&& matches, auto err)
	...
	Ownership{m_auth.id().user()}.link(
		*m_db, path_url.collection(), blob.ID(), entry, [
			location = URLIntent{URLIntent::Action::api, m_auth.id().user(), path_url.collection(), to_hex(blob.ID())}.str(),
			send = std::move(send),
			version = req.version()
		](auto ec)
		{
...
http::response<SplitBuffers> SessionHandler::not_found(boost::string_view target, unsigned version)
{
	...
	dir.emplace("username", std::string{m_auth.id().user()});
...
bool SessionHandler::renewed_auth() const
{
	return m_request_cookie.has_value()      ?  // if request cookie is present, check against the new cookie
		*m_request_cookie != m_auth.id().session() :  // otherwise, there's no cookie in the original request,
		(m_auth.valid() && !m_auth.id().is_guest()); // if we have a valid (non-guest) cookie now, then the session is renewed.
}
...
void SessionHandler::validate_collection(Collection& coll)
{
	for (auto& [id, entry] : coll.blobs())
	{
		if (entry.timestamp == Timestamp{})
		{
			...
			Ownership{m_auth.id().user()}.update(*m_db, coll.name(), id, new_entry);
		}
	}
}
