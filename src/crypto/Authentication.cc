...
void create_session(Completion&& completion, const std::string& username, redis::Connection& db)
{
	...
	db.command(
		...
		"SETEX session:%b 3600 %b", auth.cookie().data(), auth.cookie().size(), username.data(), username.size()
	);
}
...
)
{
	...
	db.command(
		...
		](redis::Reply reply, auto&& ec)
		{
			...
			// Generate session ID and store it in database
			if (!ec)
				create_session(std::move(completion), username, *db);
