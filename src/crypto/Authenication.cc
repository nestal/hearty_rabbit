...
void generate_session_id(Completion&& completion, const std::string& username, redis::Connection& db)
{
	// Generate session ID and store it in database
	// Session ID must be generated by cryptographically secure random numbrer generator.
	// Need more testing on Blake2x before using it.
	auto id = secure_random<Authentication::Cookie>();
	db.command(
		[
			id,
		...
		](redis::Reply, std::error_code ec)
		{
			...
			completion(ec, Authentication{id, username});
		},
